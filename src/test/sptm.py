...
def load_keras_model(number_of_input_frames, number_of_actions, path,
                     load_method=resnet.ResnetBuilder.build_resnet_18):
...
class InputProcessor:
    def __init__(self):
        if EDGE_NETWORK in [SIAMESE_NETWORK, JOINT_NETWORK]:
            self.edge_model = load_keras_model(number_of_input_frames=2, number_of_actions=2,
                                               path=EDGE_MODEL_WEIGHTS_PATH, load_method=EDGE_NETWORK)
            if EDGE_NETWORK == SIAMESE_NETWORK:
                self.siamese = True
                self.bottom_network = resnet.ResnetBuilder.build_bottom_network(self.edge_model,
                                                                                (NET_CHANNELS, NET_HEIGHT, NET_WIDTH))
    ...
    def predict_single_input(self, input):
        ...
        if not self.siamese:
            ...
            input_code = np.squeeze(
                self.bottom_network.predict(np.expand_dims(preprocess_images(input), axis=0), batch_size=1))
            for index in range(self.tensor_to_predict.shape[0]):
...
class SPTM:
        ...
        for first in range(len(shortcuts_matrix)):
            for second in range(first + 1 + MIN_SHORTCUT_DISTANCE, len(shortcuts_matrix)):
                ...
                for shift in range(-SHORTCUT_WINDOW, SHORTCUT_WINDOW + 1):
                    ...
                    if first_shifted < len(shortcuts_matrix) and second_shifted < len(shortcuts_matrix) \
                            and first_shifted >= 0 and second_shifted >= 0:
    ...
    def build_graph(self, keyframes, keyframe_coordinates):
        ...
        for first in range(memory_size - 1):
            ...
            self.add_double_sided_edge(first, first + 1)  # Connect each time step with the next one
    ...
    def select_IRG_on_shortest_path(self, nn, probabilities):
        ...
        if SMOOTHED_LOCALIZATION:
            ...
            for look_ahead in range(valid_min_look_ahead, valid_max_look_ahead + 1):
