...
def main_exploration(navigator, environment):
    ...
    keyframes, keyframe_coordinates, keyframe_actions = load_memory_buffer_from_lmp(memory_buffer_wad,
                                                                                    memory_buffer_lmp,
                                                                                    MEMORY_SUBSAMPLING,
                                                                                    MEMORY_MAX_FRAMES)
    return keyframes, keyframe_coordinates, keyframe_actions
...
def main_navigation(navigator, environment, mode, keyframes, keyframe_coordinates, keyframe_actions):
    ...
    for trial_index in range(NUMBER_OF_TRIALS):
        ...
        for map_index, map_name in enumerate(maps):
            ...
            for goal_index, goal_frame in enumerate(goal_frames):
                ...
                movie_filename = '%s_%s_%s_%d_%d_%s.mov' % (
                environment, mode, navigator.exploration_model_directory, trial_index, map_index, goal_name)
                ...
                goal_localization_keyframe_index = navigator.setup_navigation_test(max_number_of_steps, game,
                                                                                   goal_location, keyframes,
                                                                                   keyframe_coordinates,
                                                                                   keyframe_actions, goal_frame,
                                                                                   movie_filename,
                                                                                   TEST_SETUPS[environment].box,
                                                                                   environment)
    ...
    print('Average success path length:',
          float(sum([first * second for first, second, _ in results])) / float(max(1, number_of_successes)))
