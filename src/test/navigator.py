...
class Navigator:
    def __init__(self, exploration_model_directory):
        ...
        self.action_model = load_keras_model(number_of_input_frames=3, number_of_actions=len(ACTIONS_LIST),
                                             path=ACTION_MODEL_WEIGHTS_PATH)  # Locomotion Network
        self.memory = SPTM()
        self.trial_index = -1
    ...
    def setup_navigation_test(self, step_budget, game, goal_location, keyframes, keyframe_coordinates,
                              keyframe_actions, goal_frame, movie_path, box, environment):
        self.trial_index += 1
    ...
    def process_memory(self, environment):
        ...
        best_index, best_probability = self.memory.set_goal(self.goal_frame, self.goal_location,
                                                            self.keyframe_coordinates)
        print('Goal localization confidence:', best_probability)
        self.keyframes.append(self.goal_frame)
        self.keyframe_coordinates.append(
            self.goal_location)  # NOTE: these are not the exact goal frame coordinates, but close
    ...
    def set_intermediate_reachable_goal(self):
        ...
        self.target_index, self.nn = self.memory.find_intermediate_reachable_goal(current_screen,
                                                                                  self.game.get_state().game_variables,
                                                                                  self.keyframe_coordinates)
